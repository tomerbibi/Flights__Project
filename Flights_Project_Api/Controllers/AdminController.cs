...
namespace Flights_Project_Api.Controllers
{
    ...
    public class AdminController : FlightControllerBase<Administrator>
    {
        ...
        [HttpDelete("denyrequest/{request_id}")]
        public async Task<ActionResult> DenyRequest(int request_id)
        {
            await Task.Run(() => new AirlineAuthDAOPGSQL().Remove(request_id));
            return Ok();
        }
